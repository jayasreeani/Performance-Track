...
class Database {
    ...
    async sync() {
        try {
            const [projRes, empRes, evalRes, kpiRes, settingsRes, mentorsRes] = await Promise.all([
    ...
    async login(email, password) {
        ...
        sessionStorage.setItem('perf_eval_user_role', data.role);
    ...
    async addMentor(mentor) {
        ...
        if (!res.ok) {
            const error = await res.json();
            throw new Error(error.detail || 'Failed to add mentor');
        }
        const data = await res.json();
        await this.sync();
        return data;
    ...
    async deleteMentor(id) {
        ...
        if (!res.ok) {
            const error = await res.json();
            throw new Error(error.detail || 'Failed to delete mentor');
        }
        const data = await res.json();
        await this.sync();
        return data;
